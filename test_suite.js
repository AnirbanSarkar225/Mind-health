...
import { classifyRuleBased, classifyML, classify, onlineUpdate, computeScores } from './backend/models/classifier.js';
import { MentalState, STATE_LABELS } from './backend/models/states.js';
import { GITA_VERSES } from './backend/models/verses.js';
import { query } from './backend/config/db.js';
import { createTables } from './backend/services/schema.js';
...
async function runTestSuite() {
  ...
  await query(
    `INSERT INTO sessions 
     (id, user_id, eeg_attention, eeg_meditation, alpha_power, beta_power, theta_power, beta_alpha_ratio, detected_state, classifier_method, confidence)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  try {
    const buildOutput = execSync('npm run build --prefix frontend', { encoding: 'utf-8' });
